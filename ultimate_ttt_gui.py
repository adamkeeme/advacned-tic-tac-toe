...
class StartScreen(tk.Frame):
    def __init__(self, master: tk.Tk, launch_game_callback):
        ...
        start_button = tk.Button(self, text="Start Game", command=self._start_game_action, font=("Arial", 12))
...
class ClassicGameGUI(tk.Frame):
    ...
    def _create_board_widgets(self):
        ...
        for i in range(9):
            r, c = divmod(i, 3)
            btn = tk.Button(board_frame, text=str(i+1), font=("Arial",14,"bold"), width=4, height=2, command=partial(self._handle_player_click,i))
            btn.grid(row=r, column=c, padx=2, pady=2); self.buttons.append(btn)

    def _update_status(self, msg:str): self.status_var.set(msg)

    def _handle_player_click(self, index: int):
        if self.game_over or not self.current_player_is_human or not self.board_logic.is_empty(index): return # Uses .is_empty
        self._apply_move(index, self.player_char)
        if not self.game_over:
            self.current_player_is_human = False
            self._update_status(f"Computer ({self.ai_char}) thinking...")
    ...
    def _ai_move(self):
        ...
        self._apply_move(move, self.ai_char)
        if not self.game_over:
            self.current_player_is_human = True
            self._update_status(f"Your turn ({self.player_char})")

    def _apply_move(self, index: int, player_char: str):
        ...
        if self.board_logic.winner: # Uses .winner
            ...
            self._update_status(msg); messagebox.showinfo("Game Over", msg, parent=self)
...
class UltimateGameGUI(tk.Frame):
    def __init__(self, master: tk.Tk, difficulty: str):
        ...
        self._create_board_widgets(); self._update_board_highlights()
    ...
    def _create_board_widgets(self):
        ...
        for i in range(9):
            ...
            for j in range(9):
                sr, sc = divmod(j,3)
                btn = tk.Button(sb_frame, text=str(j+1), font=("Arial",10,"bold"), width=3,height=1, command=partial(self._handle_player_click,i,j))
                btn.grid(row=sr,column=sc,padx=1,pady=1); self.small_board_buttons[i].append(btn)

    def _update_status(self, msg:str): self.status_var.set(msg)

    def _handle_player_click(self, sb_idx: int, sq_idx: int):
        ...
        if sb_idx not in available_b: self._update_status("Invalid: Play in highlighted board."); return
        if not target_sb.is_empty(sq_idx): self._update_status("Invalid: Square taken."); return
        
        self._apply_move(sb_idx, sq_idx, self.player_char)
        if not self.game_over:
            self.current_player_is_human = False
            self._update_status(f"Computer ({self.ai_char}) thinking...")
            self.master.update_idletasks(); self.after(100, self._ai_move)

    def _ai_move(self):
        if self.game_over: return
        b_idx,s_idx = ai_bots.choose_move(self.board_logic,self.current_forced_board_idx,self.difficulty)
        self._apply_move(b_idx,s_idx,self.ai_char)
        if not self.game_over:
            self.current_player_is_human=True
            self. _update_status(f"Your turn ({self.player_char})")

    def _apply_move(self, sb_idx: int, sq_idx: int, player_char: str):
        ...
        if self.board_logic.overall_winner: 
            ...
            if self.current_player_is_human:
                ...
                if len(next_active)==1 and self.board_logic.small_boards[next_active[0]].winner is None:
                    self._update_status(f"Your turn ({self.player_char}). Play in board {next_active[0]+1}.")
                else: self._update_status(f"Your turn ({self.player_char}). Free move.")


    def _update_board_highlights(self):
        if self.game_over: return
